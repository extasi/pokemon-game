...
//creacion de clase rejilla y divs para cada personaje a partir de array


rejilla.setAttribute("class","rejilla");
game.appendChild(rejilla);

function baraja() { //para que cambien de sitio las casillas del juego metemos todo el juego en una funcion
...
function cuentaAtras(){
    ...
    if (segundos > 0){
        ...
        //else = si no / mas dependiendo el contexto
        //nombramos la funcion gameOver para darle la funcion a ejecutar mas abajo
        gameOver();
...
//funcion para ejecutar la logica de Game over

function gameOver() {
    segundos = 120;
    rejilla.classList.add("fuera");//añadimos al elemento rejilla la clase fuera
    ...
    fail.play();
    song.pause();
    setTimeout(function(){//cuando pase el tiempo ejecuta la funcion nombrada 
    while(rejilla.firstChild) {//con este evento borrariamos los dobles lo cual haria empezar de nuevo el juego
        rejilla.removeChild(rejilla.firstChild);
    }
},1000);//tiempo que debe pasar
}
...
//funcion para resetear los seleccionados cuando no coincidan
